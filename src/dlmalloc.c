...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    // mybegin
    fprintf(stderr, "SegInfo: cnt=%d, base=%p, size=%zu\n", segCount, segBase, segSize);
    // myend
...
static void* tmalloc_small(mstate m, size_t nb) {
  ...
  while ((t = leftmost_child(t)) != 0) {//查找最优节点v
    size_t trem = chunksize(t) - nb;
    if (trem < rsize) {
      rsize = trem;
      v = t;
  ...
  if (!PREACTION(ms)) {
    ...
    if (bytes <= MAX_SMALL_REQUEST) {
      ...
      if ((smallbits & 0x3U) != 0) { /* Remainderless fit to a smallbin. */
        ...
        p = b->fd;
        assert(chunksize(p) == small_index2size(idx));
        unlink_first_small_chunk(ms, b, p, idx);//gdb报错地址
...
int inmspace(mstate m, void *ptr) {
  ...
  if (!PREACTION(m)) {
    ...
    if (is_initialized(m)) {
      msegmentptr s = &m->seg;
      while (s != 0) {
        if((void*)s->base<=ptr && ptr<=(void*)(s->base+s->size)){
          POSTACTION(m);// 不知道有没有影响
          return 1;
        }
        s = s->next;
      }
...
void init(size_t size) {
  myheap = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, -1, 0);
  if (myheap == MAP_FAILED) {
    perror("init: mmap");
  }
  heap_length = size;
  ms = create_mspace_with_base(myheap, size, 0);
  mspace_track_large_chunks(ms, 1);
}

void * my_malloc(size_t bytes) {
  return mspace_malloc(myheap, bytes);
}*/
// myend